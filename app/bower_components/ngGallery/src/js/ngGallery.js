...
.directive('ngGallery', ['$document', '$timeout', '$q', '$templateCache', function($document, $timeout, $q, $templateCache) {
    ...
    '<div class="{{ baseClass }}">' +
    '  <div ng-repeat="i in images" style="display: inline-block; padding-right: 15px;">' +
    '    <div style="display: inline-block;" ng-if="!tamanioAltura && !tamanioAncho">'+
    '       <img ng-src="{{ i.thumb }}" class="{{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" />'+
    '    </div>' +
    '    <div style="display: inline-block;" ng-if="(tamanioAltura || tamanioAncho) && !dividir ">'+'' +
    '       <div>' +
	'			<img style="height: {{tamanioAltura}}px !important; width: {{tamanioAncho}}px !important;" ng-src="{{ i.thumb }}" class="{{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" />'+
    '  		</div>' +
	//	Copiar url
	'		<div ng-if="(copiarUrl != undefined)">' +
	'			 <span title="Copiar Url" tooltip data-toggle="tooltip" placement="bottom" style="height: 18px;">' +
	'			 	<label class="text-danger clickable" ' +
	'					   ngclipboard data-clipboard-target="#txtGallery{{$index}}"' +
	'					   style="font-weight: bold;max-width: {{tamanioAncho}}px; white-space: nowrap; width: {{tamanioAncho}}px; overflow: hidden; text-overflow: ellipsis;"> {{i.tipo}} </label>' +
	'			 	<input id="txtGallery{{$index}}" type="text" value="{{ i.thumb }}" style="position:absolute; top:0; left:-10000px;"/>' +
	'			 </span>' +
	'		</div>'+
    //	Fin copiar url
	'    </div>' +
    '    <div id="tres" ng-if="tamanioAltura && tamanioAncho && dividir">'+
    '       <img ng-if="$index<numero" ng-src="{{ i.thumb }}" class="{{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" />'+
    '    </div>'+

    '    <div style="display: inline-block;" ng-if="(verDetalle != undefined)">'+'' +
        '   <div>  <label ng-if="!dividir" class="text-danger" style="font-weight: bold;"> {{i.tipo}} </label>'+
        '          <label ng-if="dividir && $index<numero" class="text-danger" style="font-weight: bold"> {{i.tipo}} </label>'+
        '   </div>'+
        '	<div></div>' +
	'	</div> '+

    ' </div>' +
        '<br><div ng-if="numero && images.length>numero " style="display: inline-block; cursor: pointer; padding-top: 10px;"><i ng-click="openGallery(0)" title="Ver mas" xs-tooltip><i class="ixs ixs-agregar">Ver más</i> </i> '+
    '</div>' +
